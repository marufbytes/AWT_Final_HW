...
export default function StudentCard(props: any) {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    props.onFavoriteChange(nextState);
  };

  return (
    <div className="border border-gray-300 p-4 rounded-lg bg-white shadow-sm w-64 relative">

      <div className="absolute top-3 right-3 flex items-center gap-1 text-sm text-gray-600">
        <label htmlFor={`fav-${props.id}`} className="cursor-pointer text-xs font-medium">Favorite</label>
        <input 
          id={`fav-${props.id}`}
          type="checkbox" 
          checked={isFavorite}
